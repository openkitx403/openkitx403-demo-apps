...
def load_keypair(filepath: str = './keypair.json') -> Keypair:
    """Load Solana keypair from solana-keygen JSON file"""
    try:
        with open(filepath, 'r') as f:
            secret_key_array = json.load(f)
        
        # solders expects bytes - try different constructor
        secret_bytes = bytes(secret_key_array[:32])
        
        keypair = Keypair.from_seed(secret_bytes)
        print_success(f"Keypair loaded: {keypair.pubkey()}")
        return keypair
        ...
        print_error(f"Details: {type(e).__name__}")
        exit(1)
